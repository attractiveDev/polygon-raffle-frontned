...
const RaffleItem = (props: any) => {
  const { item, id } = props;
  console.log(id, "filter props item", item)
  const [raffle, setRaffle] = useState({...item});
  const [sellAmount, setSellAmount] = useState(0)
  console.log(id ,"filter props -raffle", raffle);
        ...
        <div className=" flex flex-col gap-[8px] bg-white p-[18px]  ">
          <div>
            ...
            <p className='text-[#1A1A1A] text-[16px] ' >
              ...
              {
                (item.discord === undefined && item.twitter === undefined && item.walletAddress !== undefined) &&
                item.walletAddress.slice(0, 3) + "..." + item.walletAddress.slice(-4)
              }
            </p>
          </div>
          ...
          <div className='border-[1px] border-dashed border-[grey] rounded-[8px] p-[10px] flex flex-col gap-[10px] ' >
            <div className="flex justify-between items-center" >
              ...
              <p className="text-sm text-[#8652FF] font-medium ">
                <span className='text-[12px]' >{item.price}</span> MATIC
              </p>
            </div>
            <div className="flex justify-between items-center " >
              <p className=" text-[#1A1A1A] text-[15px]">Tickets Remaining</p>
              <p className="text-sm text-[#8652FF] font-medium ">
                <span className='text-[12px]' > {item.total_tickets - sellAmount} </span> /{item.total_tickets}
              </p>
            </div>
            <div className="flex justify-between items-center ">
