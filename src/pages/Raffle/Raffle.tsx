...
import { SORTDATA } from '../../utils/type';
...
const Raffle = () => {
  ...
  const handleSort = (sortType: string, index: number) => {
    let tempSort: any = [], temp: any = [], sort_name_list: any = [];
    tempSort.push(sortType);
    setSortList(tempSort);
    setSelectedSort(index);
    if(isFeatured) temp = featuredData;
    if(isPastRaffles) temp = pastData;
    if(isAllRaffles) temp = raffleData;
    switch(sortType){
      case "Recently Added":
        let current_time = new Date().getTime();
        sort_name_list = temp?.filter((item: any) =>  item.start_date > current_time / 1000 - 10 * 60 * 60)
        break;
      case "Expiring Soon":
        sort_name_list = temp?.sort((a: any, b: any) => a.end_date - b.end_date);
        break;
      case "Selling Out Soon":
        sort_name_list = temp?.sort((a: any, b: any) => a.count - b.count);
        break;
      case "Price (Ascending)":
        sort_name_list = temp?.sort((a: any, b: any) => a.price - b.price);
        break;
      case "Price (Descending)":
        sort_name_list = temp?.sort((a: any, b: any) => b.price - a.price);
        break;
      case "Rank (Ascending)":
        break;
    ...
    setFilterByItem([...sort_name_list]);
  }

  const handleSearchNft = (event: any) => {
    setSearchNft(event.target.value)
    let temp: any = [];
    if(isFeatured) temp = featuredData;
    if(isPastRaffles) temp = pastData;
    if(isAllRaffles) temp = raffleData;
    const filtered_name = !event.target.value ? temp
      : temp.filter((item: any) => item.name.toLowerCase().includes(event.target.value.toLowerCase()))
  ...
  const removeSortList = (idx: number) => {
    ...
    if(isFeatured) setFilterByItem([...featuredData]);
    if(isPastRaffles) setFilterByItem([...pastData]);
    if(isAllRaffles) setFilterByItem([...raffleData]);
  }
  ...
  useEffect(() => {
    ...
    if(isPastRaffles) setSelectRaffleData([...pastData]);
    setFilterSideBar(!filterSideBar);
  }, [isFeatured, isAllRaffles, isPastRaffles, searchNft])
  ...
  return (
    <div className="bg-white min-h-[100vh] ">
      ...
      <div className="max-w-[1280px] m-auto pt-8 px-4">
        <div className="text-white rounded-[12px] bg-[#8652FF] py-[12px] px-[50px] max-w-fit  my-[0] mx-[auto] text-[24px] md:text-[30px] " >FEATURED ENDING SOON </div>
                  ...
                  <div className="flex gap-[1rem]">
                          ...
                          <ul className="ml-1">
                            {
                              SORTDATA.map((item: any, index: number) => (
                                <li className={`my-2 `} key={index}>
                                  <p
                                    onClick={() => handleSort(item, index)}
                                    className={`cursor-pointer ${index === selectedSort ? "text-[#000000]" : "text-[#5E5E5E]"} text-base hover:text-black transition-all`}
                                  >
                                    {item}
                                  </p>
                                </li>
                    ...
                    <div className={` ${filterSideBar ? `basis-[77%]` : `basis-[100%]`}`}>
                      ...
                      <div className="flex gap-[1rem] justify-left mt-4 flex-wrap pb-12 md:max-w-[768px] lg:max-w-[100%] sm:max-w-[100%] md:m-auto">
                        {isLoading ? (
                          <div id="preloader"></div>
                        ) : (Array(filterByItem) && filterByItem?.length > 0) ? (
                          filterByItem?.map((item: any, id: number) => (
                            <RaffleItem item={item} key={id} id={id}/>
                          ))
                        ) : (
